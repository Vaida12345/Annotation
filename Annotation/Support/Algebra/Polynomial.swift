...
struct Polynomial: Equatable, CustomStringConvertible, Hashable {
    ...
    var derivative: Polynomial {
        ...
        var i = -1
        while i + 1 < coefficients.count {
            i += 1
            
            coefficients[i] *= Fraction(self.degreeList[i])
        }
    ...
    var integrate: Polynomial {
        ...
        while i + 1 < coefficients.count {
            ...
            coefficients[i] /= Fraction(self.degree + 1 - i)
        }
    ...
    static func * (lhs: Polynomial, rhs: Polynomial) -> Polynomial {
        ...
        var i = -1
        while i + 1 < lhs.coefficients.count {
            i += 1
        
            var ii = -1
            while ii + 1 < rhs.coefficients.count {
                ii += 1
...
func sylvesterMatrix(_ lhs: Polynomial, _ rhs: Polynomial) -> Matrix {
    ...
    var i = -1
    while i + 1 < rhs.degree {
        i += 1
        
        var ii = -1
        while ii + 1 < lhs.degree {
            ii += 1
...
fileprivate func regression(points: [Point], degree n: Int) -> Polynomial {
    ...
    while index + 1 < points.count {
        ...
        while ii + 1 <= n {
            ...
            vector.append(pow(i.x, Fraction(ii)))
        }
        x.append(vector)
    }
...
struct RationalFunction: CustomStringConvertible {
    ...
    func reduced() -> RationalFunction {
        ...
        var index = -1
        while index + 1 < self.commonFactors.count {
            index += 1
            let i = self.commonFactors[index]
