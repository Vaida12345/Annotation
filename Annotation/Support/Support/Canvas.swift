...
//            }
            ...
            // axis texts
            var i = 0
            while i + 1 < Int(leadingTerm/getComponentDivisor(size:leadingTerm)) {
                i += 1
...
//            }
        ...
        if showGrid  {
            var i = -1
            while i + 1 < Int((domain.1-domain.0).expressedAsScientific().0/getComponentDivisor(size:(domain.1-domain.0).expressedAsScientific().0)) {
                i += 1
...
//            }
        ...
        if showGrid {
            ...
            var i = -1
            while i + 1 < Int((range.1-range.0).expressedAsScientific().0/getComponentDivisor(size:(range.1-range.0).expressedAsScientific().0)) {
                i += 1
        ...
        var index = -1
        while index + 1 < self.points.count {
            index += 1
        ...
        print(shell(["open \(FinderItem(at: path).shellPath)"]) ?? "")
    }
    ...
    init<T>(pointsFromSequence: [[T]], names: [String]? = nil) where T: BinaryFloatingPoint {
        ...
        var i = -1
        while i + 1 < pointsFromSequence.count {
            i += 1
            
            var points: [Point] = []
            var ii = -1
            while ii + 1 < pointsFromSequence[i].count {
                ii += 1
