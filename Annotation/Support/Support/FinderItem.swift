...
struct FinderItem: Codable, Copyable, CustomStringConvertible, Equatable, Hashable, Identifiable {
    ...
    ///
    /// - Note: `allChildren` are not sorted.
    ...
    var hasChildren: Bool {
        return self.findFiles() != nil && !self.findFiles()!.isEmpty
    }
    ...
    var hasSubfolder: Bool {
        guard let rawChildren = self.children else { return false }
        
        var index = -1
        while index + 1 < rawChildren.count {
            index += 1
            let i = rawChildren[index]
    ...
    func copy(to path: String) throws {
        if self.isFile {
            ...
            FinderItem(at: self.pathArray.dropLast().reduce("", { $0 + "/" + $1 })).generateDirectory()
        }
    ...
    mutating func renamed(with newName: String) throws {
        ...
        self.path = self.pathArray.dropLast().reduce("", { $0 + "/" + $1 }) + "/" + newName
    }
    ...
    /// Shows the current file in finder.
    func showInFinder() {
        if self.isDirectory {
    ...
    ///
    /// - Parameters:
    ///    - file: The folder containing the images.
    static func createIcon(at file: FinderItem) {
        file.iteratedOver { child in
            guard child.isFile else { return }
            var child = child
            guard (try? child.saveToPNG(forceSquare: true)) != nil else { return }
            let absolutePath = child.path
            
            let file = FinderItem(at: absolutePath.split(separator: "/").last!).shellPath
            let fileName = file[file.startIndex..<file.lastIndex(of: ".")!]
            let path2 = child.shellPath
    ...
    static func createPDF(fromFolder folder: FinderItem, outputPath: String = "\(NSHomeDirectory())/Downloads/PDF Output", onChangingItem: ((_ item: FinderItem)->())? = nil) {
        ...
        if folder.hasSubfolder {
            ...
            var index = -1
            while index + 1 < folder.children!.count {
                index += 1
                let i = folder.children![index]
    ...
    static func putFilesToDifferentFolders(from folder: FinderItem, folderSplitter: String = " - ", organize: Bool = false, organizeThreshold: Int = 10, ignoringKeywords: [String] = []) {
        ...
        var index = -1
        while index + 1 < children.count {
            index += 1
            let i = children[index]
    ...
    static func renameBD(from item: FinderItem, additionalOccurrence: [String] = []) {
        ...
        item.iteratedOver { item in
            ...
            var index = -1
            while index + 1 < occurrences.count {
                index += 1
                let i = occurrences[index]
