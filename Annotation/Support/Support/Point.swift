...
/// A 2D point.
struct Point: Codable, Comparable, CustomStringConvertible, Equatable, Hashable, LosslessStringConvertible {
    ...
    var cgPoint: CGPoint {
        get { return CGPoint(x: Double(self.x), y: Double(self.y)) }
        set { self.x = Point.FloatingPoint(newValue.x); self.y = Point.FloatingPoint(newValue.y) }
    }
